...
import { TermService, TaxonomyService } from "@/services";
...
export default function TermForm({ term, mode }: TermFormProps) {
  ...
  const onSubmit = async (data: TermFormData) => {
    ...
    try {
      if (mode === "create") {
        await termService.createTerm(data as CreateTermDto);
        toast.success("Term created successfully");
      } else if (term) {
        await termService.updateTerm(term.id, data as UpdateTermDto);
